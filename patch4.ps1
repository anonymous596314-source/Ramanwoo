...
$repFWD = @"
    async function fetchWordData(word) {
        try {
            ...
            // Translate all definitions and examples at once explicitly
            ...
            // first part is the title, next `parsedDefs.length` parts are translated definitions
            ...
            // remaining parts are translated examples
            let processedExamples = [];
            for (let i = 0; i < examplesRaw.length; i++) {
                processedExamples.push({
                    eng: examplesRaw[i],
...
$content = $content.Replace($findFWD, $repFWD)
