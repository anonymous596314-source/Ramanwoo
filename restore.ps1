...
$raw = [System.IO.File]::ReadAllBytes("index_utf8.html")
$html = $enc.GetString($raw)

# ── 1. Add script tag for long_articles.js before </head>
$html = $html.Replace('</head>', '<script src="long_articles.js"></script>' + "`r`n" + '</head>')
...
$newTabs = '<button id="tabDictionaryBtn" class="btn secondary-btn" style="width:auto; padding:8px 20px;">🔍 查字典</button>' + "`r`n" +
           '                <button id="tabLongArticleBtn" class="btn secondary-btn" style="width:auto; padding:8px 20px;">📚 長篇小說</button>' + "`r`n" +
           '                <button id="tabWritingBtn" class="btn secondary-btn" style="width:auto; padding:8px 20px;">✍️ 寫作區</button>' + "`r`n" +
           '                ' + $oldTabGames
$html = $html.Replace($oldTabGames, $newTabs)

# ── 3. Insert Dictionary + Long Article + Writing sections before gamesSection
$gameSectionComment = '            <!-- ?? 益智遊戲區塊 -->'
$insertBefore = $html.IndexOf('id="gamesSection"')
# Find beginning of the comment line before gamesSection
$insertIdx = $html.LastIndexOf("`n", $insertBefore)
$insertIdx2 = $html.LastIndexOf("`n", $insertIdx - 1)
...
'@
$html = $html.Substring(0, $insertIdx2 + 1) + $newSections + $html.Substring($insertIdx2 + 1)

# ── 4. Append all JS before final </script>
$lastScript = $html.LastIndexOf('</script>')
$jsToInsert = @'
    ...
    (function() {
        ...
        if(tabFlashcardBtn) tabFlashcardBtn.addEventListener('click', () => {
            ...
            tabDictionaryBtn.classList.remove('primary-btn'); tabDictionaryBtn.classList.add('secondary-btn');
            tabLongArticleBtn.classList.remove('primary-btn'); tabLongArticleBtn.classList.add('secondary-btn');
            tabWritingBtn.classList.remove('primary-btn'); tabWritingBtn.classList.add('secondary-btn');
        });
...
$html = $html.Substring(0, $lastScript) + $jsToInsert + $html.Substring($lastScript)

# ── 5. Write back as UTF-8 without BOM
$outBytes = $enc.GetBytes($html)
